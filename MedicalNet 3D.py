...
import MedicalNet
from MedicalNet import model as MedNetModel
...
#model = resnet.resnet18(sample_input_D = 32, sample_input_H = 64,  sample_input_W = 64, num_seg_classes = 2)
#print(model.parameters())

opt = {
    'model': 'resnet',  # Model type
    'model_depth': 18,  # Model depth
    'input_W': 64,     # Input width
    'input_H': 64,     # Input height
    'input_D': 32,       # Input depth (number of channels)
    'resnet_shortcut': 'A',  # Shortcut type
    'no_cuda': False,   # Use CUDA (GPU)
    'n_seg_classes': 2,  # Number of segmentation classes
    'gpu_id': [0],      # GPU ID as a list
    'phase': 'train',   # Training phase
    'pretrain_path': 'resnet_18.pth',  # Path to pretrained model
    'new_layer_names': ['fc_layer', 'conv_layer'],    # List of new layer names
}
...
class LitModel(pl.LightningModule):
    ...
    def __init__(self, num_classes, learning_rate):
        super().__init__()
        self.MedicalNetModel = MedNetModel.generate_model(opt)
        print(self.MedicalNetModel)
        self.lr = learning_rate
        

    def model(self):
        model = self.MedicalNetModel
        return model

    def forward(self, x):
        x = self.MedicalNetModel.forward(x)
    ...
    def validation_step(model, dataloader_test):
        ...
        val_pred = model.forward(val_X_batch)
        val_loss = criterion(val_pred, val_y_batch)
        
        val_accuracy = BinaryAccuracy().to(device)
        val_accuracy = val_accuracy(val_pred, val_y_batch)
...
trainer = pl.Trainer(min_epochs = 0, max_epochs = 50, log_every_n_steps = 12, precision = 16, logger = logger
